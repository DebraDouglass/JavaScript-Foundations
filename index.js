...
// monthlyRate = principal * (monthlyInterestRate * Math.pow((1 + monthlyInterestRate),periods)) / 
...
and 2 and returns a sentence "{Name}, your monthly rate is ${monthlyRate}"
...
/* Substitute the variables in your functions for parameters such that you can 
substitute `P`, `I`, and `N` when you call the function.
...
Then, add control flow within your function such that 
IF creditScore is above 740, interest rate drops by 0.5%, 
...
//     return `${name}, your monthly rate is $ ${Math.round((monthlyRate).toFixed(2))}`;
// }
...
it should console.log the monthly payment for 10 different interest rates
at 0.5% increments plus or minus 2% from the inputted interest rate. 
Complete these calculations using a for loop.*/

let name = "Debra";

function varIntRate(principal,interestRate,years) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    console.log(name + " with an interest rate of " + interestRate + ", your monthly rate is $" + Math.round(monthlyRate,2) + ".");

    interestRate = interestRate + .005;
  }
}
varIntRate(200000,0.04,30);
